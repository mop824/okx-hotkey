...
// Semantic schema: high-impact keys pinned to exact values (mirrors backend _SELECTOR_SCHEMA).
// Defense-in-depth — client rejects even if backend is compromised.
const _SELECTOR_SCHEMA = {
  submitBuy:       /^button\.okui-positivebutton$/,
  submitSell:      /^button\.okui-negativebutton$/,
  cancelAllButton: /^button\.cancel-all$/,
  cancelButton:    /^button\.btn-fill-grey$/,
};
// Generic: safe CSS chars, bare `*` rejected, max 512 chars.
const _SELECTOR_GENERIC_RE = /^(?!\*$)[\w\s\-\[\]="'.*#:,()^$|+~>/@]{1,512}$/;

function _isValidSelectorValue(key, val) {
  if (typeof val !== 'string') return false;
  if (_SELECTOR_DENY_RE.test(val)) return false;
  const schema = _SELECTOR_SCHEMA[key];
  if (schema !== undefined) return schema.test(val);
  if (!_SELECTOR_SAFE_RE.test(val)) return false;
  if (!_SELECTOR_GENERIC_RE.test(val)) return false;
  return true;
}
...
function _validatePayload(payload) {
  ...
  for (const [key, val] of Object.entries(payload)) {
    if (!_isValidSelectorValue(key, val)) return false;
  }
  return true;
}
